    ...
    <div className="mt-6 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2">
          <input className="rounded-lg border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-white placeholder:text-[var(--text-muted)]" placeholder="Search…" value={search} onChange={(e) => { setSearch(e.target.value); load(category, e.target.value); }} />
          ...
          <button onClick={() => load()} className="flex items-center gap-1 rounded-lg border border-white/10 px-3 py-2 text-xs text-[var(--text-primary)] hover:bg-white/5"><RefreshCw className="h-3 w-3" /> Refresh</button>
        </div>
      ...
      {items.length === 0 ? (
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-12 text-center text-sm text-[var(--text-muted)]">
          <Bell className="h-8 w-8 mx-auto mb-3 text-[var(--text-muted)]" /> No notifications
        </div>
      ) : (
          ...
          {items.map((n) => (
            <div key={n.id} className={`flex items-start justify-between gap-3 rounded-xl border border-white/[0.04] px-4 py-3 ${n.read ? "bg-white/[0.01]" : "bg-[var(--brand-primary)]/5"}`}>
              <div>
                ...
                <p className="mt-1 text-[10px] uppercase tracking-wide text-[var(--text-muted)]">{n.category} · {new Date(n.createdAt).toLocaleString()}</p>
              </div>
              <div className="flex flex-shrink-0 gap-1.5">
                {!n.read && <button onClick={() => archive(n.id)} title="Mark read + archive" className="rounded-md border border-white/10 p-1.5 text-[var(--text-secondary)] hover:text-[var(--text-primary)]"><Check className="h-3.5 w-3.5" /></button>}
                <button onClick={() => archive(n.id)} title="Archive" className="rounded-md border border-white/10 p-1.5 text-[var(--text-secondary)] hover:text-[var(--text-primary)]"><Archive className="h-3.5 w-3.5" /></button>
                <button onClick={() => remove(n.id)} title="Delete" className="rounded-md border border-white/10 p-1.5 text-[var(--text-secondary)] hover:text-red-400"><Trash2 className="h-3.5 w-3.5" /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
        ...
        <p className="mt-0.5 text-[11px] text-[var(--text-muted)]">Choose how you receive each category: In-app, Email, Both, or None.</p>
        <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {categories.map((c) => (
            <div key={c} className="flex items-center justify-between rounded-lg border border-white/5 bg-zinc-900/40 px-3 py-2">
              <span className="text-xs capitalize text-[var(--text-primary)]">{c.replace(/_/g, " ")}</span>
