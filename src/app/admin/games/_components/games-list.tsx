                ...
                <motion.tr key={game.id} variants={rowVariants} className="group">
                  ...
                  <td>
                    <div className="flex items-center justify-end gap-2">
                      <Link href={`${GAMES_ROUTE}/${game.id}/edit`} aria-label={`Edit ${game.name}`} className="rounded-lg p-2 text-zinc-400 transition-colors hover:bg-s8ul-cyan/10 hover:text-s8ul-cyan" title="Edit game">
                        <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                        </svg>
                      </Link>
                      <button onClick={() => handleDelete(game.id)} aria-label={`Delete ${game.name}`} className="rounded-lg p-2 text-zinc-400 transition-colors hover:bg-red-500/10 hover:text-red-400" title="Delete game">
                        <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
