...
export function PaymentsMultiproviderClient({ initialAccounts, initialActive, initialReadiness }: { initialAccounts: PaymentAccountData[]; initialActive: string | null; initialReadiness: PaymentReadinessReport | null }) {
  ...
  const verify = async (provider: string) => {
    ...
    if (!r.success) {
      ...
      setMsg(`✓ ${provider} verified and ready`);
    } else {
      const missing = r.readiness?.missing?.length ? r.readiness.missing.join(", ") : "the remaining setup steps";
      setMsg(`Provider credentials verified — storefront payments stay unavailable until you complete: ${missing}`);
    }
    ...
    <div className="mt-6 space-y-6">
      ...
      <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-card)] p-5" style={{ boxShadow: "var(--shadow-elevation)" }}>
        ...
        {(() => {
          ...
          const verifiedButBlocked = !!active && isDirect && readiness?.readiness !== "ready" && !!readiness?.missing?.length;
          if (canClaimReceiving) {
            const label = PAYMENT_PROVIDERS.find((p)=>p.id===active)?.label ?? active;
            return <p className="mt-2 text-sm text-emerald-300">You&apos;re receiving payments through: <span className="font-semibold">{label}</span> ✓</p>;
          }
          if (verifiedButBlocked) {
            return <p className="mt-2 text-sm text-amber-300">Provider credentials verified — storefront payments stay unavailable until you complete: {readiness!.missing.join(", ")}</p>;
          }
          if (active) {
            const label = PAYMENT_PROVIDERS.find((p)=>p.id===active)?.label ?? active;
            return <p className="mt-2 text-sm text-amber-300">Payments not yet ready — complete setup for {label}</p>;
          }
          return <p className="mt-2 text-sm text-amber-300">No active payment method — connect and verify a provider below.</p>;
        })()}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {PAYMENT_PROVIDERS.filter(p=>p.status==="active").map((p)=>{
          ...
          const state = !acct ? "Not connected" : acct.verificationStatus==="verified" && acct.isActive ? "Ready" : acct.verificationStatus==="verified" ? "Verified" : acct.verificationStatus==="failed" ? "Failed" : acct.verificationStatus==="pending" ? "Verifying" : "Not connected";
            return (
            <div key={p.id} className={`rounded-xl border p-4 ${acct?.isActive ? "border-[var(--color-success)]/30 bg-[var(--color-success-surface)]" : "border-[var(--border)] bg-[var(--surface-card)]"}`} style={{ boxShadow: acct?.isActive ? "var(--shadow-elevation)" : "var(--shadow-elevation)" }}>
              <div className="flex items-center gap-2">
                {p.id==="razorpay" ? <CreditCard className="h-4 w-4 text-[var(--text-secondary)]"/> : <Globe className="h-4 w-4 text-[var(--text-secondary)]"/>}
                <p className="font-semibold text-[var(--text-primary)]">{p.label}</p>
                <span className={`ml-auto rounded px-2 py-0.5 text-[10px] border ${state==="Ready"?"bg-[var(--color-success-surface)] text-[var(--color-success)] border-[var(--color-success)]":state==="Failed"?"bg-[var(--color-danger-surface)] text-[var(--color-danger)] border-[var(--color-danger)]":"bg-[var(--surface-hover)] text-[var(--text-secondary)] border-[var(--border)]"}`}>{state}</span>
              </div>
              <p className="mt-1 text-xs text-[var(--text-muted)]">{p.description}</p>
              <p className="mt-1 text-[11px] text-[var(--text-muted)]">Currency: {p.id==="stripe"?"INR, USD, EUR, GBP":"INR"} • Available in your region</p>
              {!acct ? (
                <div className="mt-3 space-y-2">
                  <input placeholder={p.id==="stripe"?"Stripe secret sk_test_...":"Razorpay Key ID"} value={getForm(p.id).providerKeyId} onChange={(e)=>setForm(p.id,"providerKeyId",e.target.value)} className="w-full rounded bg-[var(--surface-input)] border border-[var(--border)] px-2 py-1.5 text-xs text-[var(--text-primary)] placeholder:text-[var(--text-muted)]"/>
                  <input placeholder={p.id==="stripe"?"Stripe account acct_... (optional)":"Razorpay Key Secret"} type="password" value={getForm(p.id).providerKeySecret} onChange={(e)=>setForm(p.id,"providerKeySecret",e.target.value)} className="w-full rounded bg-[var(--surface-input)] border border-[var(--border)] px-2 py-1.5 text-xs text-[var(--text-primary)] placeholder:text-[var(--text-muted)]"/>
                ...
                <div className="mt-3 space-y-2">
                  <p className="text-xs text-[var(--text-secondary)]">Holder: {acct.accountHolderName ?? "—"} • {acct.hasProviderKeys?"Keys set":"No keys"}</p>
                  {acct.lastVerifiedAt && <p className="text-[11px] text-[var(--text-muted)]">Last verified: {new Date(acct.lastVerifiedAt).toLocaleString()}</p>}
                  <div className="flex flex-wrap gap-2">
                    {!acct.isActive && acct.verificationStatus==="verified" && <button disabled={busy===p.id+"-active"} onClick={()=>activate(p.id)} className="rounded bg-[var(--color-success)] px-3 py-1 text-xs text-white">Use {p.label}</button>}
                    {acct.verificationStatus!=="verified" && <button disabled={busy===p.id+"-verify"} onClick={()=>verify(p.id)} className="flex items-center gap-1 rounded border border-[var(--border)] px-3 py-1 text-xs text-[var(--text-secondary)] bg-[var(--surface-card)]"><ShieldCheck className="h-3 w-3"/> Verify</button>}
                    <button disabled={busy===p.id+"-disc"} onClick={()=>disconnect(p.id)} className="rounded border border-[var(--color-danger)]/20 px-3 py-1 text-xs text-[var(--color-danger)] bg-[var(--color-danger-surface)]">Disconnect</button>
                  </div>
                  {acct.isActive && <p className="text-xs text-[var(--color-success)] flex items-center gap-1"><CheckCircle2 className="h-3 w-3"/> Active • Future sales will use {p.label}</p>}
                </div>
              )}
            </div>
          );
