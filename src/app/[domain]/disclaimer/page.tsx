...
export async function generateMetadata({ params }: { params: { domain: string } }): Promise<Metadata> {
  ...
  if (!tenant) return {};
  const legal = await legalService.get(tenant.id, "disclaimer");
  return {
    title: `${legal.title} — ${tenant.name}`,
    description: legal.content.slice(0, 160),
    robots: { index: true, follow: true },
    alternates: { canonical: `${getCanonicalUrl(params.domain)}/disclaimer` },
  };
}

export default async function DisclaimerPage({ params, searchParams }: { params: { domain: string }; searchParams: { preview?: string } }) {
