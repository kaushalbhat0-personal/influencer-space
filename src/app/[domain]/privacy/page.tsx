...
export async function generateMetadata({ params }: { params: { domain: string } }): Promise<Metadata> {
  ...
  if (!tenant) return {};
  const legal = await legalService.get(tenant.id, "privacy");
  return {
    title: `${legal.title} — ${tenant.name}`,
    description: legal.content.slice(0, 160),
    robots: { index: true, follow: true },
    alternates: { canonical: `${getCanonicalUrl(params.domain)}/privacy` },
  };
}

export default async function PrivacyPage({ params, searchParams }: { params: { domain: string }; searchParams: { preview?: string } }) {
