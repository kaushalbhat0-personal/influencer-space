...
export default async function CommerceStrategyPage() {
  const [data, payment] = await Promise.all([getCommerceStrategyOverview(), getPaymentHealthData()]);
  if (!data.ok) return <div className="p-8 text-sm text-red-400">{data.error ?? "Unauthorized"}</div>;
  return <CommerceStrategyCenter distribution={data.distribution ?? []} migration={data.migration!} paymentHealth={payment.ok ? payment.health ?? null : null} />;
}
