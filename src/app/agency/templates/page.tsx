      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {blueprints.map((b) => (
          <div key={b.id} className="rounded-xl border border-white/10 bg-zinc-900/50 p-4">
            ...
            <span className="mt-2 inline-block rounded-full bg-white/5 px-2 py-0.5 text-[10px] text-[var(--text-secondary)]">{b.category}</span>
          </div>
        ))}
      </div>

      <h3 className="mb-3 mt-8 text-sm font-medium text-[var(--text-primary)]" data-testid="templates-themes">Themes ({themes.length})</h3>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {themes.map((t) => (
