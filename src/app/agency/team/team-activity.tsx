...
export function TeamActivity({ initialItems, initialCursor }: { initialItems: AuditItem[]; initialCursor: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-zinc-900/50 p-4">
      ...
      <p className="mb-3 text-xs text-zinc-500">Invitations, role changes and removals.</p>

      {error && <p className="mb-2 rounded-md border border-red-500/30 bg-red-500/5 px-3 py-2 text-xs text-red-400">{error}</p>}

      {items.length === 0 && !error ? (
        <p className="py-4 text-center text-sm text-zinc-500">No team activity yet.</p>
      ) : (
      ...
      {cursor && (
        <button
          onClick={loadMore}
          disabled={pending}
          className="mt-3 w-full rounded-md border border-white/10 px-3 py-1.5 text-xs text-zinc-300 hover:bg-zinc-800 disabled:opacity-40"
        >
