...
const TABS: { id: PlanFamily; label: string }[] = [
  { id: "creator", label: "For Individuals" },
    ...
    <section id="pricing" className="relative px-4 py-20 sm:px-8 sm:py-28 bg-[radial-gradient(ellipse_at_top,rgba(99,102,241,0.06),transparent_60%)] bg-zinc-900/10 border-y border-white/[0.04]">
      <div className="mx-auto max-w-7xl">
        ...
        {!isPartner && plans.some((p) => p.annualPrice) && (
          <div className="mb-8 flex items-center justify-center gap-3 text-sm" role="group" aria-label="Billing cycle" suppressHydrationWarning>
            <span suppressHydrationWarning className={cn("text-sm", cycle === "monthly" ? "text-zinc-200" : "text-zinc-500")}>Monthly</span>
            <button
              type="button"
              role="switch"
            ...
            <span className={cn("flex items-center gap-1.5 text-sm", cycle === "yearly" ? "text-zinc-200" : "text-zinc-500")} suppressHydrationWarning>
              Yearly
              {(() => {
                ...
                return maxSavings ? (
                  <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-0.5 text-xs font-medium text-emerald-400" aria-label={`Save ${maxSavings} percent with yearly billing`} suppressHydrationWarning>
                    ...
                    <div className="mt-5">
                      {trial ? (
                        ...
                        <div suppressHydrationWarning>
                          <span className="text-4xl font-bold text-white" suppressHydrationWarning>{formatCurrency(price)}</span>
                          <span className="text-zinc-500" suppressHydrationWarning>/{cycle === "yearly" ? "mo billed yearly" : "month"}</span>
                          {savings && cycle === "yearly" && (
                            <span className="ml-2 rounded-full bg-emerald-500/10 px-2 py-0.5 text-xs font-medium text-emerald-400" suppressHydrationWarning>
                              Save {savings}%
                            </span>
                          )}
                        </div>
