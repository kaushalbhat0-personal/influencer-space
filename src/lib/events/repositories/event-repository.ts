...
export class EventRepository {
  async save(event: PlatformEvent): Promise<void> {
    await tryDb(async () => {
      await prisma.platformEvent.create({
        data: { id: event.id, eventType: event.type, payload: JSON.parse(JSON.stringify(event.payload)), aggregateId: this.extractAggregateId(event), source: event.source, occurredAt: new Date(event.timestamp) },
      });
    }, undefined);
  }
  ...
  async getHistory(eventType?: string): Promise<PlatformEvent[]> {
    return tryDb(async () => {
      ...
      return records.map((r) => ({ id: r.id, type: r.eventType as EventType, payload: r.payload as PlatformEventPayloads[EventType], timestamp: r.occurredAt.toISOString(), source: r.source }));
    }, []);
  }
