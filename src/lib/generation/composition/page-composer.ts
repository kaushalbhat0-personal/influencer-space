...
export class PageComposer {
  compose(graph: KnowledgeGraph): PageBlueprint[] {
    ...
    pages.push(this.createPage("about", "About", `${slug}/about`, 4, true));
    pages.push(this.createPage("contact", "Contact", `${slug}/contact`, 5, true));

    const hasBlog = graph.creator.contentFrequency === "daily" || graph.content.postingSchedule !== "irregular";
    pages.push(this.createPage("blog", "Blog", `${slug}/blog`, 6, hasBlog));
